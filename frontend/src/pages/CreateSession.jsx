    ...
    >
      ...
      <button
        ...
        disabled={
          busy ||
          !phone.trim() ||
          (trialBlocked || noProxies || !proxyId)
        }
...
export default function CreateSession() {
  ...
  const loadProxies = useCallback(async () => {
    ...
    try {
      ...
      const rows = (r.data?.data?.proxies || []).filter((proxy) =>
        proxy.enabled !== false &&
        proxy.is_working &&
        proxy.validated_for_telegram &&
        ['socks5', 'mtproto'].includes(String(proxy.protocol || '').toLowerCase()) &&
        !(proxy.assigned_sessions?.length)
      );
      setProxies(rows);
      const auto = rows[0];
